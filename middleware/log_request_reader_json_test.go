...
func Test_NewLogRequestReaderJSON(t *testing.T) {
	...
	t.Run("error if reader is nil", func(t *testing.T) {
		if reader, err := NewLogRequestReaderJSON(nil, nil); err == nil {
	...
	t.Run("creates a new decorator", func(t *testing.T) {
		if reader, err := NewLogRequestReaderJSON(reader, nil); err != nil {
			t.Errorf("returned the (%v) error", err)
		} else if reader == nil {
	...
	t.Run("creates a new decorator with model", func(t *testing.T) {
		...
		if reader, err := NewLogRequestReaderJSON(reader, model); err != nil {
			t.Errorf("returned the (%v) error", err)
		} else if reader == nil {
			t.Errorf("didn't return a valid reference")
		} else if check := reader.(*logRequestReaderJSON).model; check != model {
			t.Errorf("stored the (%v) model", check)
		}
	})
}

func Test_LogRequestReaderJSON_Get(t *testing.T) {
	t.Run("non-json body does not add decorated field", func(t *testing.T) {
		...
		jsonReader, _ := NewLogRequestReaderJSON(reader, nil)
