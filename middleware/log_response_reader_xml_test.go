...
func Test_NewLogResponseReaderXML(t *testing.T) {
	...
	t.Run("error if reader is nil", func(t *testing.T) {
		if reader, err := NewLogResponseReaderXML(nil, nil); err == nil {
	...
	t.Run("creates a new decorator", func(t *testing.T) {
		if reader, err := NewLogResponseReaderXML(reader, nil); err != nil {
			t.Errorf("returned the (%v) error", err)
		} else if reader == nil {
	...
	t.Run("creates a new decorator with model", func(t *testing.T) {
		...
		if reader, err := NewLogResponseReaderXML(reader, model); err != nil {
			t.Errorf("returned the (%v) error", err)
		} else if reader == nil {
			t.Errorf("didn't return a valid reference")
		} else if check := reader.(*logResponseReaderXML).model; check != model {
			t.Errorf("stored the (%v) model", check)
		}
	})
}

func Test_LogResponseReaderXML_Get(t *testing.T) {
	model := struct {
	...
	t.Run("non-xml body does not add decorated field", func(t *testing.T) {
		...
		xmlReader, _ := NewLogResponseReaderXML(reader, &model)
